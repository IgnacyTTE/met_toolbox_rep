# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 16:11:47 2024
...
def instances_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq):
    curve_name = curve_key
    curve = session.get_curve(name=curve_name)
...
def tagged_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq):
    curve_name = curve_key
    curve = session.get_curve(name=curve_name)
    ts = curve.get_data(tag, data_from=date_min, data_to=date_max, function=fx, frequency=fq)
    pd_s = ts.to_pandas()
    pd_df = pd_s.to_frame()
    return pd_df

def tagged_instance_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq):
    curve_name = curve_key
    curve = session.get_curve(name=curve_name)
    ts = curve.get_instance(issue_date, tag, function=fx, frequency=fq)
    pd_s = ts.to_pandas()
    pd_df = pd_s.to_frame()
    return pd_df

def handle_curve_parameters(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq):
    ...
    if curve_type_key == 'TIME_SERIES':
        ...
        pd_df = instances_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq)
    elif curve_type_key == 'TAGGED':
        # Set parameters for a curve with tagged time series
        pd_df = tagged_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq)
    elif curve_type_key == 'TAGGED_INSTANCES':
        # Set parameters for a curve with instances and tags
        pd_df = tagged_instance_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq)
    else:
...
def fetch_voule(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq):
    
    pd_df = handle_curve_parameters(curve_key, tag, issue_date, issue_date_from, issue_date_to, date_min, date_max, fx, fq)    
