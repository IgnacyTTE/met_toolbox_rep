# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 09:54:56 2024
...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 09:45:16 2024
...
def plot_reservoir_levels(merged_df, number_of_past_years):
    ...
    for country_name in merged_df['country_name'].unique():
        ...
        df_country = merged_df[(merged_df['country_name'] == country_name) & 
                               (merged_df['Year'] >= date_min.year) & 
                               (merged_df['MonthDay'] >= date_min.strftime('%m-%d'))]
        ...
        # Plot data for the last year up to date ('Actual', 'Normal', and 'Anomaly')
        ...
        if not df_past_year.empty:
            ...
            ax.plot(pd.to_datetime(df_past_year['MonthDay'], format='%m-%d'), df_past_year['Normal'], color='black', lw=2, label=f'Normal ({past_year})')
            ax.bar(pd.to_datetime(df_past_year['MonthDay'], format='%m-%d'), df_past_year['Anomaly'], width=10, label='Anomaly', color='blue', alpha=0.5)
...
#date_max = pd.to_datetime(dict(year=today.year, month=12, day=30))
