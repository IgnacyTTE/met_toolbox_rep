# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 17:26:31 2024
...
grid_size = int(np.ceil(np.sqrt(num_countries)))  # Ensure the grid has enough space for all countries

# Setup the plot in a dynamic grid
fig, axs = plt.subplots(grid_size, grid_size, figsize=(15, 10), squeeze=False)  # Use squeeze=False for consistency
axs = axs.flatten()  # Flatten the array to make iteration easier

# Iterate over each country and plot in the dynamic arrangement
for i, country_name in enumerate(country_names):
    if i >= len(axs):  # In case there are more countries than anticipated
        break  # Optional: Extend figure or create a new figure here
    ax = axs[i]
...
fig.suptitle('Snow and Groundwater Level Status', fontsize=16, y=1.03)  # Adjust the y-position as needed based on your grid size
