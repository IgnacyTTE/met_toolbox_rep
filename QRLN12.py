# -*- coding: utf-8 -*-
"""
Created on Thu Apr 11 10:25:04 2024
...
def plot_reservoir_levels(merged_df, number_of_past_years):
    ...
    for country_name in merged_df['country_name'].unique():
        ...
        for year in range(past_year - number_of_past_years, past_year + 1):
            df_year = df_country[df_country['Year'] == year]
            if not df_year.empty:
                # Ensure only within last 12 months
                df_year = df_year[df_year['MonthDay'].isin(plot_range)]
                if year < current_year:
                    ax.plot(df_year['MonthDay'], df_year['Actual'], color='lightgray', lw=1, alpha=0.5, label='Past Data' if year == past_year - number_of_past_years else "")
                else:
                    ax.plot(df_year['MonthDay'], df_year['Actual'], color='red', lw=2, label='Actual')
                    ax.plot(df_year['MonthDay'], df_year['Normal'], color='blue', lw=2, label='Normal')
                    ax.bar(df_year['MonthDay'], df_year['Anomaly'], width=10, label='Anomaly', color='green', alpha=0.5)
        ...
        # Format the x-axis to show dates within the plotting range
