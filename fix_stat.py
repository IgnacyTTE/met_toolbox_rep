# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 15:47:37 2024
...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 11:41:43 2024
...
#%% Let's thry country_name = country_name.unique()
...
merged_df['country_name'] = merged_df['area'].apply(lambda x: get_area_name(x.upper()))
merged_df['PlotDate'] = pd.to_datetime(merged_df[['Year', 'Month', 'Day']])
years = merged_df['Year'].drop_duplicates().values 
...
for country_name in merged_df['country_name'].unique():
    ...
    #del date_idx, this_week_hist, this_week, quant_one
    
    data[country_name] = data_dict
    
data_df = pd.DataFrame(data)
data_df.to_excel('data_dict.xlsx', index=True)

#%%

data_df = pd.DataFrame(data)
data_df.to.excel('data_dict.xlsx', index=True)
...
def save_and_display_data_and_plots_to_excel(merged_df, number_of_past_years, directory):
    ...
    plots = list(plot_reservoir_levels(merged_df, number_of_past_years))  # Example: 5 years
    ...
    for idx, (country_plot, country_name) in enumerate(plots):
        ...
        image_filename = f'Reservoir_Water_Level_Status_{country_name}_{timestamp}.png'
    ...
    combined_image_filename = f'Combined_Reservoir_Water_Level_Status_{timestamp}.png'
...
def save_country_data(data, output_dir):
    ...
    filename = f"{output_dir}/res_country_data_{today}.txt"
...
#save_and_display_data_and_plots_to_excel(merged_df, number_of_past_years, directory)
...
def process_and_visualize_res_data():
...
process_and_visualize_res_data()