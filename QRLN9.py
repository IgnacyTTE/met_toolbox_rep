# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 17:04:23 2024
...
date_min = dt.date(date_max.year - 10, date_max.month, date_max.day)
fx ='AVERAGE'
fq ='D'
...
# Add a column for normalized month-day for plotting
...
for country_name in merged_df['country_name'].unique():
    ...
    for year in range(current_year - 9, current_year):
        ...
        if not df_year.empty:
            ax.plot(df_year['MonthDay'], df_year['Actual'], color='lightgray', lw=1, label='Past Data' if year == current_year - 9 else "", alpha=0.5)

    # Plot data for the past year ('Actual' and 'Normal')
    df_past_year = df_country[df_country['Year'] == past_year]
    if not df_past_year.empty:
        ax.plot(df_past_year['MonthDay'], df_past_year['Actual'], color='red', lw=2, label=f'Actual ({past_year})')
        ax.plot(df_past_year['MonthDay'], df_past_year['Normal'], color='blue', lw=2, label=f'Normal ({past_year})')
    ...
    # Adjust x-axis to show month-day format
    ax.xaxis.set_major_formatter(DateFormatter('%b-%d'))
    ax.xaxis.set_major_locator(plt.MaxNLocator(12))  # Adjust to show each month
