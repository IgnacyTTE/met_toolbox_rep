...
def time_series_curve(curve_key, tag, issue_date, issue_date_from, issue_date_to, data_from, data_to, filt, function, frequency):
    curve_name = curve_key
    curve = session.get_curve(name=curve_name)
    ts = curve.get_data(data_from, data_to, 'cet', 'cet', filt, function, frequency)
    pd_s = ts.to_pandas()
    pd_df = pd_s.to_frame()
    return pd_df
...
def handle_curve_parameters(curve_key, tag, issue_date, issue_date_from, issue_date_to, data_from, data_to, filt, function, frequency):
...
def fetch_voule(curve_key, tag, issue_date, issue_date_from, issue_date_to, data_from, data_to, filt, function, frequency):
