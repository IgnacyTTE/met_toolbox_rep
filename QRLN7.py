# -*- coding: utf-8 -*-
"""
Created on Tue Apr  9 14:43:56 2024
...
# Now use 'country_name' for grouping and plotting
country_names = merged_df['country_name'].unique()

# Setup the plot - adjust the size, number of subplots, etc., as necessary
fig, axs = plt.subplots(len(country_names), 1, figsize=(10, 5*len(country_names)))

if len(country_names) == 1:
    axs = [axs]  # Ensure axs is iterable

for ax, country_name in zip(axs, country_names):
    # Filter the data for the current country_name
    ...
    ax.bar(df_country['DateTime'], df_country['Anomaly'], width=0.05, label='Anomaly', color='blue', alpha=0.8)
...
fig.suptitle('Reservoir Water Level Status', fontsize=16)

# Adjust layout to prevent the overall title from overlapping with the subplot titles
